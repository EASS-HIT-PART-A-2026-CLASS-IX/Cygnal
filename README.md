...
Demo video: https://youtu.be/tfJD0_qhUn8
...
The backend is the single source of truth: the dashboard and enrichment
service hold no data of their own, and always read through its documented
HTTP contracts.
...
```bash
git clone https://github.com/EASS-HIT-PART-A-2026-CLASS-IX/Cygnal.git
cd Cygnal
```

Install dependencies, set up the environment file, and start the API:
...
After startup, open http://localhost:8501 and sign in with `analyst / analyst123`
or `admin / admin123`.
...
Code, tests, and documentation were written with Claude and OpenAI Codex
assistance, then reviewed and verified locally before each commit.
